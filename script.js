          ...
          .map(
            ({ title, mark }) => `
              <div class="user__courses">
                ...
                <span class="${this.renderMark(mark)}">${this.renderMark(mark)}</span></p>
              </div>
            `
          )
          .join('')
...
class Lector extends User {
  ...
  renderCourses() {
    return this.courses.length > 0
      ? this.courses
          .map(
            ({ title, score, studentsScore }) => `
              <div class="user__courses admin--info">
                  <div class="user__courses--course lector">
                      <p>Title: <b>${title}</b></p>
                      <p>Lector's score: <span class="${this.renderMark(score)}">${this.renderMark(
              score
            )}</span></p>
                      <p>Average student's score: <span class="${this.renderMark(
                        studentsScore
                      )}">${this.renderMark(studentsScore)}</span></p>
                  </div>
              </div>
            `
          )
          ...
          .map(
            ...
            <div class="user__courses admin--info">
              <div class="user__courses--course admin">
                  <p>Title: <b>${title}</b></p> 
                  <p>Admin's score: <span class="${this.renderMark(score)}">${this.renderMark(
              score
            )}</span></p>
